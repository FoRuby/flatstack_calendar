...
describe RecurringEventPolicy do
  ...
  permissions :show?, :create? do
    it 'denies access if user does not exist' do
      expect(subject).to_not permit(nil, recurring_event)
    end

    it 'grants access if user exist' do
      expect(subject).to permit(author, recurring_event)
    end
  end
  ...
  permissions :update?, :destroy? do
    ...
    it 'denies access if user not event author' do
      expect(subject).to_not permit(not_author, recurring_event)
    end
